...
public class LTDescr
{
    ...
    public LTDescr setColorParticle()
    {
        this.type = TweenAction.COLOR;
        this.initInternal = () =>
        {
            if (trans.GetComponent<ParticleSystemRenderer>() != null && trans.GetComponent<ParticleSystemRenderer>().material.HasProperty("_Color"))
            {
                Color col = trans.GetComponent<Renderer>().material.color;
                this.setFromColor(col);
            }
        ...
        this.easeInternal = () =>
        {
            ...
            // Debug.Log("val:"+val+" tween:"+tween+" tween.diff:"+tween.diff);
            if (this.type == TweenAction.COLOR)
                colorRecursiveParticle(trans, toColor, this.useRecursion);
            if (dt != 0f && this._optional.onUpdateColor != null)
                this._optional.onUpdateColor(toColor);
            else if (dt != 0f && this._optional.onUpdateColorObject != null)
                this._optional.onUpdateColorObject(toColor, this._optional.onUpdateParam);
    ...
    public LTDescr setBlur()
    {
        this.type = TweenAction.ALPHA;
        this.initInternal = () => { this.fromInternal.x = this.trans.GetComponent<UnityEngine.UI.Image>().material.GetFloat("_Size"); };
        this.easeInternal = () => { this.trans.GetComponent<UnityEngine.UI.Image>().material.SetFloat("_Size", easeMethod().x); };
        return this;
    }

    public LTDescr setMatColor()
    {
        this.type = TweenAction.COLOR;
        this.initInternal = () =>
        {
            this.uiImage = trans.GetComponent<UnityEngine.UI.Image>();
            this.setFromColor(this.uiImage.material.color);
        };
        this.easeInternal = () =>
        {
            newVect = easeMethod();
            ...
            Color toColor = tweenColor(this, val);
            toColor.a = 0;
            this.uiImage.material.color = toColor;
        };
        return this;
    }
    ...
    public LTDescr setLayoutScale()
    {
        this.type = TweenAction.SCALE;
        this.initInternal = () => { this.fromInternal.x = trans.GetComponent<UnityEngine.UI.LayoutElement>().preferredWidth; };
        this.easeInternal = () =>
        {
            newVect = easeMethod();
            trans.GetComponent<UnityEngine.UI.LayoutElement>().preferredWidth = newVect.x;
        };
        return this;
    }
	...
	*/
    ...
    private static void colorRecursiveParticle(Transform transform, Color toColor, bool useRecursion = true)
    {
        Renderer ren = transform.gameObject.GetComponent<ParticleSystemRenderer>();
        if (ren != null)
            foreach (Material mat in ren.materials)
                mat.color = toColor;
        ...
        if (useRecursion && transform.childCount > 0)
        {
            foreach (Transform child in transform)
            {
                colorRecursiveParticle(child, toColor);
            }
        }
    }

    private static void colorRecursiveMaterial(Transform transform, Color toColor, bool useRecursion = true)
    {
        UnityEngine.UI.Image ren = transform.gameObject.GetComponent<UnityEngine.UI.Image>();
        if (ren != null)
        ...
        {
            foreach (Transform child in transform)
                colorRecursiveMaterial(child, toColor);
        }
    }
    ...
    private static void imageAlphaRecursive(Transform trans, float val, bool useRecursion = true)
    {
        UnityEngine.UI.Image uiImage = trans.GetComponent<UnityEngine.UI.Image>();
        if (uiImage != null)
        {
